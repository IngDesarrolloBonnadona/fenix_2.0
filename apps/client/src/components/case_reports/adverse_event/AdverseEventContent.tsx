...
const AdverseEventContent = () => {
  ...
  const [showDescriptionOthersLocalState, setShowDescriptionOthersLocalState] =
    useState(false);
  const [
    descriptionAdverseEventLocalState,
    setDescriptionAdverseEventLocalState,
  ] = useState("");
  ...
  const idOfCaseTypeState = useAppSelector(
    (state) => state.changeOfCaseType.idOfCaseTypeAdverseEvent
  );
  ...
  const handleChangeEvent = async (value: number) => {
    ...
    try {
      ...
      if (response && response.data) {
        ...
        setFindSimilarReportDataLocalState(response.data.data || []);
      }
    } catch (error) {
  ...
  const handleClickFindPatientData = async () => {
    ...
    try {
      ...
      if (isSuccess && !isError) {
        ...
        setFindAdmissionsPatientDataLocalState(isSuccess.admissions);
        ...
        form.validateFields(["consecutive-patient"]);
      }
    } catch (error) {
    ...
    try {
      ...
      const response: any = await createCaseReportOriginal({
        ...
        newReportOriginal: {
          ...
          ori_cr_description: descriptionAdverseEventLocalState,
  ...
  return (
    <div className="case-report-adverse-event" style={{ padding: "16px" }}>
            ...
            <Col xs={24} sm={12} md={8} lg={6}>
              ...
              >
                <Select
                  placeholder={"Seleccione una opción"}
                  onChange={(value) => {
                    setOriginServiceIdLocalState(value);
                  }}
            ...
            <Row gutter={[16, 16]} style={{ width: "100%" }}>
              <Col xs={24} sm={25} md={12} lg={24}>
                <Form.Item
                  ...
                  id="description-adverse-event"
                  className="description-adverse-event"
                  name="description-adverse-event"
                  style={{
                    width: "100%",
                    marginTop: "10px",
                    marginBottom: 0,
                ...
                >
                  <TextArea
                    ...
                    value={descriptionAdverseEventLocalState}
                    style={{ width: "100%" }}
                  />
                </Form.Item>
              </Col>
...
export default AdverseEventContent;
