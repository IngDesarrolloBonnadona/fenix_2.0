...
import { useUpdateProtocolMutation } from "@/redux/apis/protocol/protocolApi";

const EditProtocolButtonComponent: React.FC<{
  dataRecord: Protocol;
  ...
  const [updateProtocol, { isLoading: updateProtocolDataLoading }] =
    useUpdateProtocolMutation();

  useEffect(() => {
    if (isModalOpen) {
      setNameLocalState(dataRecord.prot_name);
      setDescriptionLocalState(dataRecord.prot_description);
    }
  ...
  const hasChanges = () => {
    const initialData = {
      dataName: dataRecord.prot_name,
      dataDescription: dataRecord.prot_description,
    };
  ...
  const handleClickSubmit = async () => {
    try {
      const response: any = await updateProtocol({
        id: dataRecord.id,
        updateProtocol: {
  ...
  return (
    <>
      ...
      <CustomModalNoContent
        key={"custom-modal-edit-protocol"}
        ...
        contentCustomModal={
          <>
            <Form
              form={form}
              id="edit-protocol-form"
              name="edit-protocol-form"
              className="edit-protocol-form"
              initialValues={{
                "name-protocol": dataRecord.prot_name,
                "description-protocol": dataRecord.prot_description,
              }}
              autoComplete="off"
              layout="vertical"
              style={{ width: "100%" }}
            ...
            >
              ...
              >
                <CustomButton
                  classNameCustomButton="edit-protocol-button"
                  idCustomButton="edit-protocol-button"
                  titleCustomButton="Actualizar"
                  typeCustomButton="primary"
                  htmlTypeCustomButton="submit"
                  iconCustomButton={
                    !updateProtocolDataLoading ? (
                      <BiEdit />
                    ) : (
                      <LoadingOutlined />
                    )
                  ...
                  disabledCustomButton={
                    hasChanges() && !updateProtocolDataLoading ? false : true
                  }
                  onClickCustomButton={() => ({})}
                  styleCustomButton={{
                    color: "#ffffff",
...
export default EditProtocolButtonComponent;
