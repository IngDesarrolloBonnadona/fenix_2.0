...
import { useDeleteRiskConsequenceMutation } from "@/redux/apis/risk_consequence/riskConsequence";

const DeleteRiskConsequenceButtonComponent: React.FC<{
  onRefetch: () => void;
  dataRecordId: number;
}> = ({ onRefetch, dataRecordId }) => {
  const dispatch = useDispatch();

  const [deleteRiskConsequence] = useDeleteRiskConsequenceMutation();

  const handleClickDelete = async (id: number) => {
    try {
      const response: any = await deleteRiskConsequence(id);
...
export default DeleteRiskConsequenceButtonComponent;
