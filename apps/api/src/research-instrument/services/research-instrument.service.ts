...
export class ResearchInstrumentService {
  ...
  async updateResearchInstrument(
    ...
    if (updateResearchInstrumentDto.inst_r_name) {
      const duplicateName = await this.researchInstrumentRepository.findOne({
        where: {
          id: Not(id),
          inst_r_name: updateResearchInstrumentDto.inst_r_name,
          inst_r_status: true,
        },
      });
      ...
      if (duplicateName) {
        throw new HttpException(
          `El nombre del instrumento de investigación ya existe.`,
          HttpStatus.CONFLICT,
        );
      }
    }
