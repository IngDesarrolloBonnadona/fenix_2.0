...
import { RiskLevelEnum } from 'src/utils/enums/risk-level.enum';
...
export class RiskLevelService {
  ...
  async findAllRiskLevel() {
    ...
    const orderMap: Record<RiskLevelEnum, number> = {
      [RiskLevelEnum.HIGH]: 1,
      [RiskLevelEnum.MIDDLE]: 2,
      [RiskLevelEnum.LOW]: 3,
    };

    return riskLevels.sort(
      (a, b) =>
        orderMap[a.ris_l_name as RiskLevelEnum] -
        orderMap[b.ris_l_name as RiskLevelEnum],
    );
  }
  ...
  async updateRiskLevel(id: number, updateRiskLevelDto: UpdateRiskLevelDto) {
    ...
    if (updateRiskLevelDto.ris_l_name) {
      const duplicateName = await this.riskLevelRepository.findOne({
        where: {
          id: Not(id),
          ris_l_name: updateRiskLevelDto.ris_l_name,
          ris_l_status: true,
        },
      });
      ...
      if (duplicateName) {
        throw new HttpException(
          `El nombre del nivel de riesgo ya existe.`,
          HttpStatus.CONFLICT,
        );
      }
    }
