...
import { ReportAnalystAssignmentService } from 'src/report-analyst-assignment/services/report-analyst-assignment.service';
import { SynergyService } from 'src/synergy/services/synergy.service';
import { ResearchersService } from 'src/report-researchers-assignment/services/report-researchers-assignment.service';
import { CharacterizationCasesService } from 'src/characterization-cases/services/characterization-cases.service';
...
import { ObservationReturnCaseService } from 'src/observation-return-case/services/observation-return-case.service';
...
import { CreateCaseReportOriginalDto } from 'src/case-report-original/dto/create-case-report-original.dto';
...
export class CaseReportValidateService {
  ...
  async findSimilarCaseReportsValidate(
    ...
    if (month) {
      startDate = dayjs().subtract(month, 'month').startOf('day').toDate();
    }
    ...
    const similarReport = await this.caseReportValidateRepository.find({
      where: {
        ...
        ...(startDate
          ? { val_cr_creationdate: Between(startDate, today) }
          : {}),
      },
      relations: {
        caseType: true,
    ...
    try {
      ...
      if (
        ...
        const severityClasificationFound = await queryRunner.manager.findOne(
          ...
          {
            where: {
              sev_c_name: SeverityClasificationEnum.MODERATE_SEVERITY,
            },
          },
        );
      ...
      caseReportValidate.val_cr_severityclasif_id_fk =
        createReportValDto.val_cr_severityclasif_id_fk;
      ...
      caseReportValidate.val_cr_creationdate =
        previousReport.val_cr_creationdate;

      const caseReportValidateSave =
      ...
      return new HttpException(
        {
          message: `¡El reporte con el consecutivo #${caseReportValidate.val_cr_filingnumber} se validó exitosamente.!`,
          data: caseReportValidateSave,
        },
        HttpStatus.CREATED,
      );
    } catch (error) {
  ...
  async createReportValidateTransaction(
    ...
    const caseReportValidate = this.caseReportValidateRepository.create({
      ...
      val_cr_creationdate: caseReportOriginal.ori_cr_creationdate,
  ...
  async summaryReportsLastMont(months: number) {
    const today = dayjs().endOf('day').toDate();
    let startDate: Date | undefined;

    if (months) {
      startDate = dayjs().subtract(months, 'month').startOf('day').toDate();
    }
