...
export class RiskTypeService {
  ...
  async updateRiskType(id: number, updateRiskTypeDto: UpdateRiskTypeDto) {
    ...
    await this.findOneRiskType(id);

    if (updateRiskTypeDto.ris_t_name) {
      const duplicateName = await this.riskTypeRepository.findOne({
        where: {
          id: Not(id),
          ris_t_name: updateRiskTypeDto.ris_t_name,
          ris_t_status: true,
        },
      });
      ...
      if (duplicateName) {
        throw new HttpException(
          `El nombre del tipo de riesgo ya existe.`,
          HttpStatus.CONFLICT,
        );
      }
    }
