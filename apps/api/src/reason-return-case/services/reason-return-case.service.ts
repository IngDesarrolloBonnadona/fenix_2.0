...
export class ReasonReturnCaseService {
  ...
  async updateReasonReturnCase(
    ...
    if (updateReasonReturnCaseDto.rec_r_cause) {
      const duplicateName = await this.reasonReturnCaseRepository.findOne({
        where: {
          id: Not(id),
          rec_r_cause: updateReasonReturnCaseDto.rec_r_cause,
          rec_r_role_id_fk: updateReasonReturnCaseDto.rec_r_role_id_fk,
          rec_r_status: true,
        },
      });
      ...
      if (duplicateName) {
        throw new HttpException(
          `El nombre del motivo de devolución ya existe con este rol.`,
          HttpStatus.CONFLICT,
        );
      }
    }
