...
export class MovementReportService {
  ...
  async updateMovementReport(
    ...
    if (updateMovementReportDto.mov_r_name) {
      const duplicateName = await this.movementReportRepository.findOne({
        where: {
          id: Not(id),
          mov_r_name: updateMovementReportDto.mov_r_name,
          mov_r_status: true,
        },
      });
      ...
      if (duplicateName) {
        throw new HttpException(
          `El nombre del movimiento ya existe.`,
          HttpStatus.CONFLICT,
        );
      }
    }
