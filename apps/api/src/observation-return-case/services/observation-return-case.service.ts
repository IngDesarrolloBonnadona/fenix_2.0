...
import { ReasonReturnCaseService } from 'src/reason-return-case/services/reason-return-case.service';
...
export class ObservationReturnCaseService {
  ...
  async findOneObservationReturnByCaseValidateId(CaseValidateId: string) {
    if (!CaseValidateId) {
      throw new HttpException(
        'El identificador del caso es requerido.',
        HttpStatus.BAD_REQUEST,
    ...
    const observationReturn = await this.observationReturnRepository.findOne({
      where: {
        rec_o_validatedcase_id_fk: CaseValidateId,
      },
      withDeleted: false,
      relations: {
        reasonReturnCase: true,
      },
    });
